...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    # files via cp/rsync.
    #
    # Why sudo (not setuid / setcap)?
    #   - File caps (`setcap cap_sys_admin+ep`) trip util-linux mount's
    #     "drop permissions failed" guard.
    #   - setuid-root mount voluntarily drops privileges when invoked by a
    #     non-root real uid and then only honours mounts present in
    #     /etc/fstab with `user`/`users` options — which dynamic loop
    #     mounts aren't.
    #   - sudo wraps the invocation so both real AND effective uid become 0
    #     before mount sees them; mount then operates as a normal root
    #     mount (no drop, no fstab restriction). The accompanying sudoers
    #     entry locks the allowlist to exactly the binaries the convert
    #     worker needs.
    exfatprogs util-linux mount \
    && rm -rf /var/lib/apt/lists/* \
    # Give uid 1000 a real /etc/passwd entry. Required by sudo (which
    # otherwise prints "you do not exist in the passwd database") and by
    # rsync's --info=progress2 (which queries getpwuid). Home dir is
    # /home/p5manager so any tooling that looks at $HOME doesn't fall back
    # to "/" and try to write dot-files into the rootfs.
    && useradd -u 1000 -m -s /bin/sh -d /home/p5manager p5manager \
    # NOPASSWD sudoers entry pinned to uid 1000. Restricted to the loop +
    # exfat binaries so the cap escalation is bounded to the exfat
    # pipeline; nothing else in the manager can wander through sudo.
    && printf '%s\n' \
       '# Auto-generated by Dockerfile — allows the convert worker (uid 1000) to' \
       '# drive losetup/mount/umount/mkfs.exfat for the PS5 exFAT image pipeline.' \
       '# Container also needs cap_add: SYS_ADMIN + seccomp:unconfined from' \
       '# docker-compose.yml; without those caps these binaries would fail at' \
       '# the syscall layer regardless of sudo.' \
       'p5manager ALL=(root) NOPASSWD: /usr/sbin/losetup, /usr/bin/mount, /usr/bin/umount, /usr/sbin/mkfs.exfat' \
       > /etc/sudoers.d/p5manager-exfat \
    && chmod 0440 /etc/sudoers.d/p5manager-exfat \
