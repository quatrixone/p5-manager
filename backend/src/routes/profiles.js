...
router.post('/', (req, res) => {
  try {
    const { name, ip_address, mac_address, port, credential } = req.body;
    ...
    const db = getDatabase();
    db.run(
      'INSERT INTO profiles (name, ip_address, mac_address, port, credential) VALUES (?, ?, ?, ?, ?)',
      [name, ip_address, mac_address || null, port || 9021, credential || null]
    );
...
router.put('/:id', (req, res) => {
  try {
    ...
    db.run(
      'UPDATE profiles SET name = ?, ip_address = ?, mac_address = ?, port = ?, credential = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
      [name || existing.name, ip_address || existing.ip_address, mac_address !== undefined ? mac_address : existing.mac_address, port || existing.port, credential !== undefined ? credential : existing.credential, parseInt(id)]
    );
